...
const procesarTrabajo = async (trabajo) => {
    ...
    switch (trabajo.tipo_trabajo) {
        case 'enviar_email_invitacion_residente':
            ...
            const { destinatarioEmail, destinatarioNombre, token, nombreEdificio } = trabajo.payload;
            // Llamamos a nuestro servicio de email para ejecutar la tarea.
            await emailService.enviarEmailInvitacionResidente(destinatarioEmail, destinatarioNombre, token, nombreEdificio);
            break;
        default:
...
const cicloDelWorker = async () => {
    ...
    try {
        ...
        console.log(`Se encontraron ${trabajos.length} trabajos para procesar.`);
